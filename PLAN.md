...
| | |
|---|---|
| ~~**Where the audio lives**~~ | **Decided: a container.** `.cutpkg` is a zip holding `project.json` and an `assets/` folder. Every option considered was a trade against the founding property - reference by path and a project stops being one file; inline under a size cap and the format silently changes shape at a threshold nobody can see. A container dissolves it instead: the single-file property is exactly preserved, bytes are stored as bytes, and the result is *smaller* than the JSON (measured: 267 KB → 201 KB on a 200 KB payload). The manifest is the same `CutProject` schema, so loading either shape produces the same object and nothing downstream knows which it came from. The extension decides, never a size. `.cut.json` stays for everything without a payload in it, because readable and diffable is worth keeping for the common case. |
